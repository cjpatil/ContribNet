...
class CompositionDatasetActivations(CompositionDataset):
    ...
    def __init__(self, root, pmodel, phase, split):
        super().__init__(root, phase, split)

        # precompute the activations -- weird. Fix pls
        feat_file = '%s/%s_features.t7'%(root,pmodel)
        if not os.path.exists(feat_file):
            with torch.no_grad():
                self.generate_features(pmodel, feat_file)
    ...
    def generate_features(self, pmodel, out_file):
        ...
        if pmodel == "resnet18":
            ...
            feat_extractor = tmodels.vgg16(pretrained=True)
        elif pmodel == "vgg19":
            feat_extractor = tmodels.vgg19(pretrained=True)
        elif pmodel == "resnet101":
            feat_extractor = tmodels.resnet101(pretrained=True)
        elif pmodel == "resnet152":
            feat_extractor = tmodels.resnet152(pretrained=True)
        elif pmodel == "densenet161":
            feat_extractor = tmodels.densenet161(pretrained=True)

        feat_extractor.eval().cuda()

        image_feats = []
        image_files = []
        for chunk in tqdm.tqdm(utils.chunks(data, 128), total=len(data)//128):
